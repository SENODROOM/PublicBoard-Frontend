...
export default function Home() {
  ...
  useEffect(() => {
    ...
    donationsAPI
      .getStats()
      .then((r) => setDonationStats(r.data))
      .catch(() => {});
    issuesAPI
      .getAll({ sort: "-createdAt", limit: 3 })
      .then((r) => setRecentIssues(r.data.issues.slice(0, 3)))
      .catch(() => {});
  }, []);
  ...
  const categories = [
    {
      icon: "🏗️",
      name: "Infrastructure",
      desc: "Roads, streetlights, potholes",
    },
    ...
    { icon: "🗑️", name: "Sanitation", desc: "Trash, waste management" },
    {
      ...
      desc: "Shared equipment, supplies",
    },
    ...
    <div>
        ...
        <div className="container" style={{ position: "relative" }}>
          <div style={{ maxWidth: 720 }}>
            ...
            >
              Your Voice.
              <br />
              <span style={{ color: "var(--amber)" }}>Your Neighborhood.</span>
              <br />
              Your Board.
            </h1>
            ...
            >
              Report problems, track resolutions, and support your community —
              all in one transparent platform.
            </p>

            <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
              <Link to="/report" className="btn btn-amber btn-lg">
                Report an Issue
              </Link>
              <Link
                to="/dashboard"
            ...
            <div className="grid-3">
              {recentIssues.map((issue) => (
                <Link to={`/issues/${issue._id}`} key={issue._id}>
                  ...
                  >
                    <span
                      className={`badge badge-${issue.status === "Open" ? "open" : issue.status === "In Progress" ? "progress" : issue.status === "Resolved" ? "resolved" : "pending"}`}
      ...
      >
        <div className="container" style={{ textAlign: "center" }}>
          <h2 style={{ color: "var(--white)", fontSize: 40, marginBottom: 16 }}>
          ...
          >
            Pool resources with neighbors to fund solutions, organize events,
            and make lasting improvements.
          </p>
          <Link
            to="/donate"
            className="btn btn-lg"
            style={{ background: "var(--amber)", borderColor: "var(--ink)" }}
          >
            Make a Donation
          </Link>
        </div>
            ...
            {[
              {
                num: "01",
                title: "Report",
                desc: "Submit detailed info about your concern — location, category, description.",
              },
              {
                num: "02",
                title: "Display",
                desc: "Your issue goes public on the dashboard, visible to the whole community.",
              },
              {
                num: "03",
                title: "Update",
                desc: "Track progress as reporters and admins update the status in real-time.",
              },
              {
                num: "04",
                title: "Resolve",
                desc: "Celebrate wins! Resolved issues are archived for community records.",
              },
            ].map((step, i) => (
              <div
                key={i}
                style={{ padding: 24, borderLeft: "4px solid var(--amber)" }}
              >
                <div
                  style={{
                    fontFamily: "var(--font-display)",
